...
def toyless_blue_cats
  ...
  execute(<<-SQL)
    SELECT cats.name
    FROM cats
    LEFT OUTER JOIN cattoys ON cattoys.cat_id = cats.id
    WHERE cats.color = 'Navy Blue' AND cattoys.toy_id IS NULL;
  SQL
  # 63 with cattoys.cat_id index
end
...
def cats_like_johnson
  ...
  # indexes
  # CREATE INDEX cats_breed_color on cats(color, breed);

  # with indexes on color/breed/name = 102
  execute(<<-SQL)
    SELECT cats.name
    FROM cats
    WHERE cats.breed = (
...
def cats_with_a_lot
  ...
  # change join to sub query for COUNT(toys.id)
